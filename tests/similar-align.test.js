...
describe('alignHashSequences', () => {
    it('covers identical sequences fully with mean Hamming 0', () => {
        const a = seq(['1', '2', '3', '4']);
        const r = alignHashSequences(a, seq(['1', '2', '3', '4']));
    ...
    it('aligns through a bumper/intro on the longer side', () => {
        const parent = seq(['b', '1', '2', '3', '4']);
        const clip = seq(['1', '2', '3', '4']);
        const r = alignHashSequences(parent, clip, { matchHamming: 50 });
        expect(r.matched).toBe(4);
        expect(r.coverageB).toBe(1);
    ...
    it('places an excerpt inside a parent and reports parent offset', () => {
        const parent = seq(['1', '2', '3', '4', '5']);
        const clip = seq(['3', '4']);
        const r = alignHashSequences(clip, parent, { matchHamming: 50 });
    ...
    it('allows a skipped scene via a gap', () => {
    ...
    it('rejects unrelated sequences', () => {
        const r = alignHashSequences(seq(['a', 'b', 'c', 'd']), seq(['1', '2', '3', '4']), {
            matchHamming: 50,
        });
    ...
    it('returns an empty result for missing sequences', () => {
        const r = alignHashSequences([], seq(['1']));
        expect(r.matched).toBe(0);
